...
def read(self): # Đổi "grad" thành "read"
    """
    Lấy một khung hình (frame) từ camera.

    Returns:
        tuple[bool, numpy.ndarray | None]: Trả về (True, frame) nếu thành công, (False, None) nếu thất bại.
    """
    ...
    # Trả về cả ret và frame để tương thích với cv2.read()
    return ret, frame
    ...
    def release(self):
        ...
        """
        if self.isOpened() and self.cap is not None:
            self.cap.release()
