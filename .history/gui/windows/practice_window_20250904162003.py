...
def connect_camera(self, index):
    self.disconnect_camera() # Ngắt kết nối cũ trước
    self.cam = Camera(index)
    
    # === ÁP DỤNG LẠI LOGIC KIỂM TRA KÉP TỪ CODE CŨ ===
    # isOpened() kiểm tra kết nối phần mềm.
    # read()[0] kiểm tra xem có đọc được frame thực tế không.
    if self.cam.isOpened() and self.cam.read()[0]:
        self.video_timer.start(30)
        logger.info(f"Đã kết nối và xác thực thành công camera tại chỉ số {index}.")
    else:
        logger.error(f"Kết nối camera {index} thất bại: Mở được nhưng không đọc được frame.")
        self.disconnect_camera("Không thể lấy hình ảnh từ camera.")
