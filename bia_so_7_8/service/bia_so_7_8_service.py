...
from bia_so_7_8.model.bia_so_7_8_model import warp_image
import logging

logger = logging.getLogger(__name__)

DEFAULT_ORIGINAL_PATH = "images/original/original_bia_so_4.jpg" # thay sau
DEFAULT_MASK_PATH = "images/mask/mask_bia_so_4.jpg"     # thay sau

class BiaSo7_8Service:
    def __init__(
        self,
        ...
        if self.original is None:
            logger.error(f"[BiaSo7-8Service] Không đọc được ảnh gốc tại '{self.original_path}'")
        self.mask = cv2.imread(self.mask_path, 0)
        if self.mask is None:
            logger.error(f"[BiaSo7-8Service] Không đọc được mask tại '{self.mask_path}'")
