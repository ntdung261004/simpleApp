...
class BluetoothTrigger(QObject):
    ...
    def on_press(self, key):
        ...
        if self.is_active and key == self.trigger_key and not self._is_key_pressed:
            ...
            logger.info(f"Trigger (pynput): Phát hiện tín hiệu từ phím {key}")
            # Emit signal trong luồng của pynput, Qt sẽ tự động queue sang luồng chính
            self.triggered.emit()
    ...
    def activate(self):
        ...
        logger.info("Trigger: Đã BẬT lắng nghe.")
        self.is_active = True

    def deactivate(self):
        """Tắt chức năng lắng nghe."""
        logger.info("Trigger: Đã TẮT lắng nghe.")
    ...
    def start_global_listener(self):
        ...
        if self.listener is None:
            ...
            self.listener.start()
            logger.info(f"Trigger: Luồng lắng nghe phím bấm toàn cục đã bắt đầu.")
    ...
    def stop_global_listener(self):
        ...
        if self.listener is not None:
            ...
            self.listener.join()
            self.listener = None
            logger.info("Trigger: Luồng lắng nghe phím bấm toàn cục đã dừng.")