...
from PySide6.QtCore import Qt, QSize, QPoint, QByteArray
...
class VideoLabel(QLabel):
    def __init__(self, parent=None):
        ...
        self.aspect_ratio = 3.0 / 4.0 # Tỷ lệ rộng:cao
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.setAlignment(Qt.AlignCenter)

    # Các hàm này sẽ báo cho layout biết cách tính chiều cao dựa trên chiều rộng
    ...
    def paintEvent(self, event):
        ...
        # Bây giờ widget đã có tỷ lệ đúng, ta chỉ cần scale ảnh cho vừa vặn
...
class MainGui(QWidget):
    ...
    def update_results(self, result_frame):
        """Chỉ cập nhật thời gian và ảnh kết quả, xóa thông tin điểm."""
        self.time_label.setText(f"Thời gian: {datetime.now().strftime('%H:%M:%S')}")
        self.target_name_label.setText("Tên mục tiêu: --")
