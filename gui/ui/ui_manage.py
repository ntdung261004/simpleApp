...
class VideoLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if not self._pixmap.isNull():
            ...
            # Nếu không có ảnh, để cho QLabel tự vẽ text "Chưa có ảnh"
...
class ManageGui(QWidget):
    ...
    def _create_left_column(self) -> QWidget:
        ...
        soldier_box = QGroupBox("Danh sách Người học"); soldier_layout = QVBoxLayout(soldier_box)
        self.soldier_table = QTableWidget(0, 2); self.soldier_table.setHorizontalHeaderLabels(["Họ và Tên", "Lớp"])
    ...
    def _create_analysis_box(self) -> QGroupBox:
        ...
        # === THÊM MỚI: BỎ THANH CUỘN VÀ SET CHIỀU CAO CỐ ĐỊNH ===
        ...
        # Tính chiều cao dựa trên 3 dòng, mỗi dòng cao 32px
        self.analysis_target_table.verticalHeader().setDefaultSectionSize(32)
        table_height = header_height + (3 * 32) + 2 # +2 cho đường viền
    ...
    def _create_shot_preview_box(self) -> QGroupBox:
        ...
        # VideoLabel tự co giãn, chỉ cần thêm nó vào layout với stretch > 0
        self.result_image = VideoLabel()
        layout.addWidget(self.result_image, 1) # stretch = 1
    ...
    def _create_right_column(self) -> QWidget:
        ...
        data_widget = QWidget(); data_layout = QVBoxLayout(data_widget)
        data_layout.setContentsMargins(0, 0, 0, 0); data_layout.setSpacing(15) # Giảm margin để đồng bộ
        history_box = QGroupBox("Lịch sử bắn của Người học được chọn"); history_layout = QVBoxLayout(history_box)
        self.history_list = QListWidget(); history_layout.addWidget(self.history_list); data_layout.addWidget(history_box)
        
        message_widget = QWidget(); message_layout = QVBoxLayout(message_widget)
        self.right_message_label = QLabel("..."); self.right_message_label.setAlignment(Qt.AlignCenter)
