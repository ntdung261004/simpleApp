...
# Lớp VideoLabel giữ nguyên, không thay đổi
...
# === LỚP MỚI: QLabel LUÔN HIỂN THỊ ẢNH HÌNH VUÔNG ===
class SquareImageLabel(QLabel):
    """Một QLabel tùy chỉnh để hiển thị QPixmap luôn giữ tỷ lệ 1:1 (vuông)."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMinimumSize(50, 50) # Kích thước tối thiểu
        self.setAlignment(Qt.AlignCenter)

    def setPixmap(self, pixmap: QPixmap):
        # Lưu pixmap gốc và cập nhật hiển thị
        self._pixmap = pixmap
        super().setPixmap(self._scaled_pixmap())

    def resizeEvent(self, event):
        # Khi widget thay đổi kích thước, tính toán và vẽ lại pixmap
        if hasattr(self, '_pixmap') and not self._pixmap.isNull():
            super().setPixmap(self._scaled_pixmap())

    def _scaled_pixmap(self) -> QPixmap:
        # Lấy kích thước nhỏ nhất (giữa chiều rộng và chiều cao) để làm cạnh hình vuông
        size = min(self.width(), self.height())
        # Scale pixmap theo kích thước đó mà không làm méo hình
        return self._pixmap.scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
...
class CompetitionGui(QWidget):
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QGroupBox {
                font-size: 16px; font-weight: bold; border: 1px solid #4a6278;
                border-radius: 8px; margin-top: 10px;
            }
            QGroupBox::title {
                subcontrol-origin: margin; subcontrol-position: top center;
                padding: 2px 12px; background-color: #415a72; border-radius: 4px;
            }
            QPushButton {
            ...
            VideoLabel { 
                ...
                border-radius: 8px; color: #95a5a6; font-size: 24px; 
            }
        """)
        ...
        title_label = QLabel("MÀN HÌNH THI ĐẤU")
        ...
        # Khôi phục đúng tỷ lệ cột 3:4:3
    ...
    # Các hàm _create_participants_column và _create_camera_column giữ nguyên
    def _create_participants_column(self) -> QWidget:
        panel = QGroupBox("Danh sách Xạ thủ"); layout = QVBoxLayout(panel)
        layout.setContentsMargins(15, 25, 15, 15); layout.setSpacing(10)
        self.participants_list = QListWidget()
        self.participants_list.setStyleSheet("font-size: 14px; border: 1px solid #4a6278;")
        layout.addWidget(self.participants_list, 1)
        self.back_button = QPushButton("Về Menu Thi đấu"); self.back_button.setObjectName("danger")
        layout.addWidget(self.back_button)
        return panel

    def _create_camera_column(self) -> QWidget:
        panel = QGroupBox("Đường ngắm Trực tiếp"); layout = QVBoxLayout(panel)
        layout.setContentsMargins(15, 25, 15, 15); layout.setSpacing(10)
        self.camera_view_label = VideoLabel(); self.camera_view_label.setText("Vui lòng kết nối camera")
        layout.addWidget(self.camera_view_label, 1)
        controls_panel = QWidget(); controls_layout = QVBoxLayout(controls_panel)
        controls_layout.setContentsMargins(0, 5, 0, 0); controls_layout.setSpacing(5)
        ...
        controls_layout.addLayout(sliders_layout); controls_layout.addLayout(buttons_layout)
        layout.addWidget(controls_panel)
        return panel

    # === HÀM _create_score_column ĐÃ ĐƯỢC CẬP NHẬT ===
    def _create_score_column(self) -> QWidget:
        panel = QGroupBox("Bảng điểm")
        panel_layout = QVBoxLayout(panel)
        panel_layout.setContentsMargins(15, 25, 15, 15)
        self.score_stack = QStackedWidget()
        panel_layout.addWidget(self.score_stack) # Không cần stretch factor ở đây nữa
        ...
        self.start_turn_button.setMinimumSize(200, 60)
        self.start_turn_button.setStyleSheet("font-size: 16px;")
        ...
        shooter_info_box = QGroupBox("Thông tin Xạ thủ")
        ...
        targets_grid_layout.setSpacing(10)
        ...
        self.total_score_label.setFont(font_summary)
        self.ammo_count_label.setFont(font_summary)
        ...
        scoreboard_layout.addLayout(targets_grid_layout, 1) # Cho lưới chiếm không gian thừa
    ...
    # === HÀM _create_target_widget ĐÃ ĐƯỢC TỐI ƯU ===
    def _create_target_widget(self, title: str, image_path: str) -> tuple[QWidget, QLabel]:
        """Hàm trợ giúp tạo widget bia với ảnh luôn là hình vuông."""
        ...
        # Sử dụng SquareImageLabel thay vì QLabel thông thường
        image_label = SquareImageLabel()
        image_label.setPixmap(QPixmap(image_path))
        
        score_label = QLabel("Điểm: --")
        score_label.setAlignment(Qt.AlignCenter)
        score_label.setStyleSheet("font-weight: bold; font-size: 14px; color: #f1c40f;")

        layout.addWidget(image_label, 1) # Cho ảnh chiếm phần lớn không gian
        layout.addWidget(score_label)
        
        return widget, score_label
